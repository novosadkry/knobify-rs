...
/// A baseline at least this old is not trusted: read it back first.
///
/// Confirmed sends keep the baseline fresh, so this only ever delays the first
/// tick of a turn - the one that would jump.
pub const DEFAULT_STALE_AFTER: Duration = Duration::from_secs(1);
...
impl Readback {
    ...
    fn is_stale(&self, now: Instant) -> bool {
        self.synced_at
            .is_none_or(|at| now.saturating_duration_since(at) >= self.stale_after)
    }
